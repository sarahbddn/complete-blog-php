...
function createPost($request_values)
{
    ...
    if (empty($body)) {
        array_push($errors, "Du texte est requis");
    }
    if (empty($topic_id)) {
        array_push($errors, "Une catégorie est requis");
    }
    // Get image name
    $featured_image = $_FILES['featured_image']['name'];
    if (empty($featured_image)) {
        array_push($errors, "Une miniature est requise");
    }
    // image file directory
    $target = "../static/images/" . basename($featured_image);
    if (!move_uploaded_file($_FILES['featured_image']['tmp_name'], $target)) {
        array_push($errors, "Echec de la publication de l'image.");
    }
...
if (isset($_GET['publish']) || isset($_GET['unpublish'])) {
    ...
    if (isset($_GET['publish'])) {
        ...
        $message = "Article retiré des publications.";
